...
yq -i '."cactus-parser".secretData.AUTH_JWT_PROFILE = strenv(AUTH_JWT_PROFILE)' "$(dirname "$0")/../values.yaml"
yq -i '."cactus-parser".secretData.APP_SECRET = strenv(APP_SECRET)' "$(dirname "$0")/../values.yaml"
yq -i '."cactus-parser".envVars.APP_KEY = strenv(APP_KEY)' "$(dirname "$0")/../values.yaml"
yq -i '."cactus-parser".envVars.AUKI_API_URL = "https://api.auki.network"' "$(dirname "$0")/../values.yaml"
yq -i '."cactus-parser".envVars.AUKI_DDS_URL = "https://dds.auki.network"' "$(dirname "$0")/../values.yaml"
yq -i '."cactus-parser".envVars.AUTH_SERVER_URL = "https://auth.auki.network"' "$(dirname "$0")/../values.yaml"
