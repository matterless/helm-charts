...
* Semantic event changes can be sent to an external webhook by setting `EVENTS_ENABLED: true` and providing your webhook endpoint via `EVENTS_URL`, webhook auth token via `EVENTS_AUTH_TOKEN` in your values or environment variables.
