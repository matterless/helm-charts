...
* Semantic events can be sent to an external webhook by setting `EVENTS_ENABLED: true` and providing your webhook endpoint via `EVENTS_URL`, webhook auth token via `EVENTS_AUTH_KEY` in Cactus Backend's environment variables or secrets. By default, semantic events are processed and sent every 5 minutes. If you need to adjust this frequency, update `CRONJOBS_PROCESS_EVENTS_SCHEDULE` in the values or environment variables.
